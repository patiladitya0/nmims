...
export default function Maps() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5001/api/user/account', config); // Replace with your actual API endpoint
  ...
  const fetchCrises = async () => {
    try {
      const response = await axios.get('http://localhost:5001/crises'); // Fetch crises from your backend
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5001/crisis', {
        desc,
        fullName,
        time,
        date,
